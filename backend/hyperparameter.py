...
def get_hyperparameters(network_type):
	if network_type == 'mlp':
		network = MaskedMLP([784, 500, 500, 500, 500, 10]).to(device)
		...
		network = MaskedAlexNet().to(device)
		prune_ratios = [.15, .2, .2, .2, .2, .2, .2, .2]
		...
		network = MaskedVGG11(use_bn=True).to(device)
		...
		network = MaskedVGG16(use_bn=True).to(device)
		...
		network = MaskedVGG19(use_bn=True).to(device)
		...
		network = MaskedResNet18().to(device)
		...
		network = MaskedVGG19_64(use_bn=True).to(device)
		prune_ratios = [.15] * 16 + [.10]
		optimizer = partial(optim.Adam, lr=0.0003)
		pretrain_iteration = 60000
		finetune_iteration = 40000
		...
		network = MaskedResNet50_64().to(device)
		...
		network = MaskedWideResNet_64(16, 8).to(device)
